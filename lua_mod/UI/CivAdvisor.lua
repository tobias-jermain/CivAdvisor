-- CivAdvisor.lua  —  FULL BOARD exporter
-- Reads the whole readable game state each turn and prints it to Lua.log as a
-- sectioned snapshot the overlay reassembles. Client-side only, no gameplay
-- effect, multiplayer-safe. EVERY game call is pcall-guarded via safe()/safeStr()
-- so a missing API just becomes 0/"unknown" — it can never crash your turn.
--
-- Snapshot format (one entity per line, so no line ever gets too long):
--   CIV_ADVISOR_SNAP_BEGIN turn=N
--   CIV_ADVISOR_P  {player ...}
--   CIV_ADVISOR_C  {city ...}      (repeated)
--   CIV_ADVISOR_U  {unit ...}      (repeated)
--   CIV_ADVISOR_R  {rival ...}     (repeated)
--   CIV_ADVISOR_S  {city-state ...}(repeated)
--   CIV_ADVISOR_SNAP_END turn=N
-- Plus CIV_ADVISOR_TURNEND:1 when you end your turn (overlay auto-hides).
...
local function onCityProductionChanged(playerID)
    if playerID == Game.GetLocalPlayer() then serializeState(false) end   -- light: skip heavy scans
end
...
print("CIV_ADVISOR_LOADED: CivAdvisor mod active (full-board build)")
